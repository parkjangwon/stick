...
- 🛡️ **안전한 구조적 변환 (Safe Rename)**: macOS의 APFS/HFS+ 특성을 고려하며, 대상 이름의 파일이 이미 존재하는 파일 충돌 상황 시 덮어쓰지 않고 `_conflict` 꼬리표를 달아 원본 데이터 유실을 완벽히 방지합니다. 변환 시 반드시 **하위(Leaf)부터 상위(Root) 순서**로 정밀하게 처리합니다.

<div align="center">
  <h1>✨ stick ✨</h1>
  <p><strong>macOS 한글 파일명 NFC 정규화 도구 🇰🇷</strong></p>
...
- ⚙️ **백그라운드 데몬**: macOS(`launchd`) 서비스 매니저를 활용하여 백그라운드에서 안정적으로 실행됩니다.
...
stick config
```
> 터미널에서 방향키와 `Enter`를 사용해 직관적으로 감시 폴더를 추가하고, 제외할 확장자나 디렉토리를 설정할 수 있습니다. 설정은 프로그램을 종료할 때 자동으로 안전하게 저장됩니다.
...
로그 파일은 기본적으로 `~/logs/stick/` 디렉토리에 일별로 로컬 시간 기준으로 저장됩니다. (`stick_YYYY-MM-DD.log`)
